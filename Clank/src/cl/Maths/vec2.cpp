...
namespace cl { 
	...
	vec2 API operator+(vec2 left, const vec2& right)
	{
	...
	vec2 API operator+(vec2 left, float32 value)
	{
	...
	vec2& vec2::operator+=(float32 value)
	{
		return Add(value);
	}
	...
	vec2& vec2::Lerp(const vec2& other, float32 t)
	{
		x += (other.x - x) * t;
		y += (other.y - y) * t;
	...
	vec2 vec2::Lerp(const vec2& start, const vec2& end, float32 t)
	{

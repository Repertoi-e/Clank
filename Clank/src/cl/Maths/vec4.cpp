...
namespace cl { 
	...
	float32 vec4::Dot(const vec4& other)
	{
		return x * other.x + y * other.y + z * other.z + w * other.w;
	}

	vec4& vec4::Lerp(const vec4& other, float32 t)
	{
		x += (other.x - x) * t;
		y += (other.y - y) * t;
		z += (other.z - z) * t;
	...
	vec4 vec4::Lerp(const vec4& start, const vec4& end, float32 t)
	{
		vec4 result = vec4();
		result.x += (end.x - start.x) * t;
		result.y += (end.y - start.y) * t;
		result.z += (end.z - start.z) * t;
		result.w += (end.w - start.w) * t;

...
void Universe::SetPreset(const String& preset)
{
	...
	if (preset == L"-glider") { SetAlive(250, 250); SetAlive(251, 251); SetAlive(251, 252); SetAlive(250, 252); SetAlive(249, 252); }if (preset == L"-spaceship")
	{
		...
		(254, 251); SetAlive(253, 250);
	}if (preset == L"-exploder") {
		SetAlive(250, 250); SetAlive(250, 251); SetAlive(250, 252); SetAlive(250, 253); SetAlive(250
			, 254); SetAlive(254, 250); SetAlive(254, 251); SetAlive(254, 252); SetAlive(254, 253); SetAlive(254, 254); SetAlive(252, 254); SetAlive(252, 250);
	}if (preset
		== L"-exploder_small") {
		SetAlive(250, 250); SetAlive(249, 251); SetAlive(249, 252); SetAlive(250, 252); SetAlive(250, 253); SetAlive(251, 251); SetAlive(251
			, 252);
	}if (preset == L"-tumbler") {
		SetAlive(250, 250); SetAlive(251, 250); SetAlive(255, 250); SetAlive(256, 250); SetAlive(250, 251); SetAlive(252, 251);
		SetAlive(254, 251); SetAlive(256, 251); SetAlive(250, 252); SetAlive(252, 252); SetAlive(254, 252); SetAlive(256, 252); SetAlive(252, 253); SetAlive(254,
			...
			(255, 255);
	}if (preset == L"-gosper_glider_gun") {
		SetAlive(250, 250); SetAlive(250, 251); SetAlive(250, 252); SetAlive(251, 252); SetAlive(252, 251); SetAlive
		...
		(271, 248); SetAlive(258, 243); SetAlive(259, 242); SetAlive(258, 244); SetAlive(259, 244); SetAlive(260, 244);
	}if (preset == L"-ten_cell_row") {
		SetAlive(247,
...
void Universe::Draw(Renderer2D* renderer, const cl::vec2& offset)
{
	...
	for (u32 x = 0; x < (UNIVERSE_SIZE); x++)
	{
		for (u32 y = 0; y < (UNIVERSE_SIZE); y++)
		{
			...
			cell->bounds.position = vec2(x, y) * CELL_RENDER_SIZE + vec2(CELL_RENDER_SIZE) / 2 - (vec2(CELL_RENDER_SIZE) * UNIVERSE_SIZE) / 3;
